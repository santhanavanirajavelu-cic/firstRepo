...
const CertificationC1 = require("./CertificationC1.json");

async function main() {
    //Grab your Hedera testnet account ID and private key from your .env file
    // const accountId1 = AccountId.fromString(process.env.ACCOUNT1ID);
    // const accountKey1 = PrivateKey.fromStringED25519(process.env.ACCOUNT1_PRIVATE_KEY);
    const accountId1 = process.env.ACCOUNT1ID;
    const accountKey1 = process.env.ACCOUNT1_PRIVATE_KEY;
    ...
    client.setOperator(accountId1, accountKey1);

   //Get the contract bytecode
    const bytecode = CertificationC1.abi.bytecode;
    ...
    const associateToken = new ContractExecuteTransaction()
        ...
        .setFunction("tokenAssociate", new ContractFunctionParameters()
             ...
             .addAddress(accountId1.toSolidityAddress())
             //The token ID to associate to the account
             .addAddress(tokenId.toSolidityAddress()));

    //Sign with the account key and submit to the Hedera network
    const signTx = await associateToken.freezeWith(client).sign(accountKey1);
